...
def list_fuel_entries(vehicle_id: int | None = None, db: Session = Depends(get_db)):
    q = select(FuelEntry).order_by(FuelEntry.datum)
...
def delete_fuel_entry(entry_id: int, db: Session = Depends(get_db)):
